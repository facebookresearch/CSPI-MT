...
for t in range(len(gammas)):
    ...
    for i in range(nsims):
        ...
        if t == 0:
            data = generate_data_roof(n=5 * n_tune, mu_0_var = 5, mu_1_var = 5, cov_noise = 5, propensity = 0.5, n_folds = 5)
        else:
            data = generate_data_roof(n=5 * n_tune, mu_0_var = 5/2, mu_1_var = 5/2, cov_noise = 5/2, propensity = 0.5, n_folds = 5)
    ...
    expected_improvement = [
        np.mean(pass_HCPI_finites *  true_policy_diff_roof(HCPI_finites[:, 0])),
        np.mean(pass_HCPI_asymps *  true_policy_diff_roof(HCPI_asymps[:, 0])),
        np.mean(
            pass_single_cutoffs[:, 0]
            * true_policy_diff_roof(single_cutoffs[:, 0])),
        np.mean(
            pass_single_cutoffs[:, 1]
            * true_policy_diff_roof(single_cutoffs[:, 1])
        ),
        np.mean(
            pass_single_cutoffs_dr[:, 0]
            * true_policy_diff_roof(single_cutoffs_dr[:, 0])
        ),
        np.mean(
            pass_single_cutoffs_dr[:, 1]
            * true_policy_diff_roof(single_cutoffs_dr[:, 1])
        ),
        np.mean(value_mt_2),
        np.mean(value_mt_3),
        np.mean(naive_pass * true_policy_diff_roof(naive_pass_cutoffs))
    ]
...
""":py"""
labels = ['HCPI (t-test)', 'CSPI-P', 'CSPI', 'CSPI-MT-E', 'CSPI-MT']
pass_rates_hsnr = pass_rates_data[1, :]
ei_hsnr = expected_improvement_data[1, :]

pass_rates_lsnr = pass_rates_data[0, :]
ei_lsnr = expected_improvement_data[0, :]


pass_rates_h = np.append(pass_rates_hsnr[1:2], pass_rates_hsnr[4:8])
ei_h = np.append(ei_hsnr[1:2], ei_hsnr[4:8])

pass_rates_l = np.append(pass_rates_lsnr[1:2], pass_rates_lsnr[4:8])
ei_l = np.append(ei_lsnr[1:2], ei_lsnr[4:8])
...
ax.bar(x - np.repeat(bar_width, len(labels))/2, pass_rates_h, bar_width, label='High SNR')
ax.bar(x + np.repeat(bar_width, len(labels))/2, pass_rates_l, bar_width, label='Low SNR')
# Set title and labels
ax.set_title('Pass Rates over Different SNR Regimes')
ax.set_xlabel('Method')
ax.set_ylabel('Pass Rates')
ax.set_xticks(x)
...
# Create the bar plots
ax.bar(
    x - np.repeat(bar_width, len(labels)) / 2,
    ei_h,
    bar_width,
    label="High SNR",
    color="purple",
...
# Set title and labels
ax.set_title("Expected Improvement over Different SNR Regimes")
ax.set_xlabel("Method")
